...
## 程式流程
1. `ATI` 確認 NB303 是否可回應。
2. `AT+CEREG?` 每 5 秒檢查註網，最長 300 秒。
3. `AT+EDNS="mqttgo.io"` 解析 MQTT Domain。
4. `AT+EMQNEW=218.161.43.149,1883,60000,1024` 建立 MQTT 連線實體。
5. `AT+EMQCON=0,3.1,"6545451212",60000,0,0,"",""` 連線到 Broker。
6. 進入週期任務：
   - 先讀取 PZEM004T。
   - 讀值成功才組 JSON 並發送。
   - 讀值失敗每 2 秒重試，不等待 60 秒。
   - MQTT 發送失敗則回到步驟 1 做完整恢復。
...
## Node-RED 成果
Node-RED Dashboard（電流/電壓/功率/度數）：

![Node-RED Dashboard](./10_nbiot_energy_mqtt_v60/docs/nodered-dashboard.jpg)

Node-RED Flow（訂閱 `csu/energy/data` 後解析並顯示）：

![Node-RED Flow](./10_nbiot_energy_mqtt_v60/docs/nodered-flow.jpg)

### Node-RED 圖片說明
- Dashboard 圖：四個儀表分別顯示 `A`、`V`、`W`、`kWh`，下方折線圖顯示時間趨勢。
- Flow 圖：`MQTT in -> json -> payload.A/V/W/kWh -> gauge/chart`。
- 告警圖塊：`電流通知上限`、`電流異常` 與 `10 mins` 用於避免重複通知。
